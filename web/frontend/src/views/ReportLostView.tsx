...
import { Upload, Loader2, CheckCircle, Bell, Search, ArrowRight, X, MapPin, Clock, FileText, Camera, Sparkles } from 'lucide-react';
...
export default function ReportLostView() {
  ...
  return (
    <div className="max-w-4xl mx-auto" dir={dir}>
                ...
                <div className="bg-[#28B3A3]/10 rounded-xl p-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{t('modal_lost_step1_title')}</p>
                        <p className="text-sm text-gray-600">
                          {t('modal_lost_step1_desc')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{t('modal_lost_step2_title')}</p>
                        <p className="text-sm text-gray-600">
                          {t('modal_lost_step2_desc')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{t('modal_lost_step3_title')}</p>
                        <p className="text-sm text-gray-600">
                          {t('modal_lost_step3_desc')}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {matches.filter(m => m.similarity >= 0.7).length > 0 && (
                  <div className="bg-[#28B3A3]/10 border-2 border-[#28B3A3] rounded-xl p-4">
                    <p className="text-[#0B2D3A] font-semibold text-center">
                      {t('modal_matches_found_message').replace('{count}', matches.filter(m => m.similarity >= 0.7).length.toString())}
                    </p>
                  </div>
                )}
              </div>
      ...
      {/* Hero Header */}
      <div className="relative mb-6 sm:mb-8 bg-gradient-to-br from-[#0B2D3A] via-[#0d3847] to-[#28B3A3] rounded-2xl sm:rounded-3xl p-5 sm:p-8 lg:p-10 text-white overflow-hidden shadow-xl">
        <div className="hidden sm:block absolute top-0 right-0 w-64 h-64 bg-[#28B3A3]/10 rounded-full -translate-y-1/2 translate-x-1/2"></div>
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-3 sm:mb-4">
            <div className="bg-[#28B3A3]/30 backdrop-blur-sm p-2 sm:p-3 rounded-xl sm:rounded-2xl shadow-lg">
              <Search className="w-6 h-6 sm:w-8 sm:h-8" />
            </div>
            <div>
              <h1 className="text-xl sm:text-3xl lg:text-4xl font-bold">{t('lost_page_title')}</h1>
              <p className="hidden sm:block text-[#28B3A3]/90 text-sm sm:text-base mt-0.5 sm:mt-1">{t('lost_page_subtitle')}</p>
            </div>
          </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8 border border-gray-100">
          ...
          <div className={`border-2 border-dashed rounded-xl sm:rounded-2xl p-5 sm:p-8 text-center transition-all ${formData.imagePreview ? 'border-[#28B3A3] bg-[#28B3A3]/10' : 'border-gray-300 hover:border-[#28B3A3] hover:bg-[#28B3A3]/5'}`}>
            {formData.imagePreview ? (
              ...
              <label className="cursor-pointer block">
                ...
                <p className="text-gray-800 font-medium mb-1">{t('lost_upload_click')}</p>
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-[#0B2D3A] to-[#28B3A3] text-white py-4 rounded-xl font-bold hover:from-[#0d3847] hover:to-[#36C2B2] transition-all disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed flex items-center justify-center gap-3 shadow-lg hover:shadow-xl hover:shadow-[#28B3A3]/30"
        >
          {isSubmitting ? (
            <>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                  ...
                  {t('lost_ai_matches_title')}
                </h2>
                <p className="text-gray-600 mt-1">{t('lost_ai_confidence_note')}</p>
              </div>
            </div>
            ...
            {filteredMatches.length > 0 ? (
              <div className="space-y-4">
                <div className="bg-[#28B3A3]/10 border border-[#28B3A3] rounded-lg p-4 mb-4">
                  <p className="text-[#0B2D3A] font-medium">
                    {t('lost_matches_count_message').replace('{count}', filteredMatches.length.toString())}
                  </p>
                  <p className="text-gray-700 text-sm mt-1">
                    {t('lost_matches_review_hint')}
                  </p>
                </div>
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">
                  {t('lost_no_matches_title')}
                </h3>
                <p className="text-gray-600 max-w-md mx-auto mb-6">
                  {t('lost_no_matches_desc')}
                </p>
                
                <div className="bg-[#28B3A3]/10 border border-[#28B3A3] rounded-lg p-4 max-w-lg mx-auto">
                  <p className="text-[#0B2D3A] font-medium mb-2">{t('lost_pro_tip_title')}</p>
